...
`&quality=low`. `&quality=auto` exercises the adaptive controller. The query
override wins over a saved preference without replacing it. During normal play,
press `Q` to cycle Auto → High → Medium → Low; that selection is persisted.
